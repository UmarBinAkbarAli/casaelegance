...
// Rate tables sourced from "Casa_Elegance Cost Calculator (Final).xlsx" — Rate Tables sheet.
...
const COST_CALCULATOR_WEBHOOK_URL =
  "https://services.leadconnectorhq.com/hooks/JEkJ58Cm0Kcdq5GM29uk/webhook-trigger/2MGyvbwiTruBUBZCfKVE";
...
function setupCostCalculator() {
  ...
  const showResult = () => {
    ...
    resultFields.cost.textContent = `${formatCurrency(estimate.lower)} - ${formatCurrency(estimate.upper)}`;
  ...
  if (sendOtpButton && !REQUIRE_OTP_VERIFICATION) {
    sendOtpButton.textContent = "See My Estimate";
  }
  ...
  const resetLeadPanel = () => {
    ...
    if (sendOtpButton) { sendOtpButton.disabled = false; sendOtpButton.textContent = REQUIRE_OTP_VERIFICATION ? "Send Verification Code" : "See My Estimate"; }
